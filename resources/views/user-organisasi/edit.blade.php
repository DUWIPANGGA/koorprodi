...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden p-6 mb-8 border border-gray-300">
        <div class="text-center">
            <h1 class="text-3xl font-bold text-gray-800 mb-2">Edit Keaktifan Organisasi</h1>
            <p class="text-base text-gray-600">Perbarui organisasi untuk <span class="font-semibold text-gray-900">{{ $user->name }}</span> pada Semester {{ $user->semester }}</p>
            <div class="mt-4 flex justify-center">
                <div class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-full shadow-sm">
                    ...
                    <span class="text-sm text-gray-700">Perbarui organisasi yang anda ikuti dengan sebenar benarnya</span>
                </div>
            </div>
        </div>
    </div>

    <form action="{{ route('user-organisasi.update', $user->id) }}" method="POST" class="bg-white rounded-xl shadow-md p-6 border border-gray-200">
        @csrf
        @method('PUT')
                    ...
                    <div class="flex items-start">
                        <div class="flex items-center h-5 mt-1">
                            <input class="h-4 w-4 text-gray-700 border-gray-300 rounded focus:ring-gray-500" 
                                   ...
                                   @if(in_array($organisasi->id, $selectedOrganisasiIds)) checked @endif>
                        </div>
                        <div class="ml-3 flex-1">
                            <label for="org_{{ $organisasi->id }}" class="block">
                                ...
                                <div class="mt-3 org-jabatan-field @if(!in_array($organisasi->id, $selectedOrganisasiIds)) hidden @endif">
                                    ...
                                    <input type="text" 
                                           ...
                                           value="{{ $selectedOrganisasi[$organisasi->id]->jabatan ?? '' }}"
                                           @if(in_array($organisasi->id, $selectedOrganisasiIds)) required @endif>
                                </div>
                            </label>
                        </div>
                    </div>
