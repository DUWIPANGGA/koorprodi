...
<div class="min-h-screen p-4 md:p-8">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-600 mt-2">Detailed information about {{ $user->name }}</p>
        </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <div class="p-6">
                    <div class="flex flex-col items-center mb-6">
                        ...
                        <p class="text-gray-500">{{ $user->prodi }}</p>
                    </div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="font-medium">{{ $user->angkatan }}</p>
                                </div>
                                <div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p class="font-medium">{{ $user->hobi }}</p>
                                    </div>
                                    <div>
                                        <p class="text-xs text-gray-500">Talents</p>
                                        <p class="font-medium">{{ $user->bakat }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-gray-50 px-6 py-4 flex justify-between items-center border-t border-gray-200">
                    ...
                    <div class="flex space-x-3">
                        ...
                        <form id="statusForm" method="POST" action="{{ route('rekap.user', $user->id) }}">
                            @csrf
            ...
            <div>
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="divide-y divide-gray-200">
                        @forelse ($user->rekap as $rekap)
                        <div class="p-6">
                            <div class="flex justify-between items-start">
                                <div>
                                    <h4 class="font-medium text-gray-900">Semester {{ $rekap->semester }}</h4>
                                    <div class="mt-2 flex items-center">
                                        <span class="text-2xl font-bold text-gray-900 mr-2">{{ $rekap->IPK }}</span>
                                        <span class="px-2 py-1 text-xs rounded-full 
                                            {{ $rekap->IPK >= 3.5 ? 'bg-green-100 text-green-800' : 
                                               ($rekap->IPK >= 3.0 ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800') }}">
                                            {{ $rekap->IPK >= 3.5 ? 'Excellent' : 
                                               ($rekap->IPK >= 3.0 ? 'Good' : 'Average') }}
                                        </span>
                                    </div>
                                </div>
                                <div class="flex space-x-2">
                                    <a href="{{ asset($rekap->dokumen) }}" target="_blank" class="inline-flex items-center px-3 py-1 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                    ...
                                    <span class="inline-flex items-center px-3 py-1 rounded-md text-sm font-medium 
                                        {{ $rekap->validated == 1 ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                        {{ $rekap->validated == 1 ? 'Verified' : 'Pending' }}
                                    </span>
                                </div>
                            </div>
                            @if($rekap->catatan)
                            <div class="mt-4 p-3 bg-blue-50 rounded-md">
                                <p class="text-sm text-blue-700">{{ $rekap->catatan }}</p>
                            </div>
                            @endif
                        </div>
                        @empty
